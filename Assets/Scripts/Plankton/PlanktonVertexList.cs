...
namespace Plankton
{
    ...
    public class PlanktonVertexList : IEnumerable<PlanktonVertex>
    {
        ...
        {
            PlanktonVertex pv =  new PlanktonVertex(vertex.X, vertex.Y, vertex.Z);
            //pv.normal = normal;
            this._list.Add(pv);
            return this.Count - 1;
        }
       ...
       /* /// <summary>
        ...
        public int[] AddVertices(IEnumerable<PlanktonXYZ> vertices,IEnumerable<PlanktonXYZ> normals)
        {
            int i = 0;
            foreach(var v in vertices)
            {
                this.Add(v, normals[i]);
            }
